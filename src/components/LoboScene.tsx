...
export const LoboScene: React.FC<LoboSceneProps> = ({ mood, onLoboTap }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-2.5 select-none">
      {/* 1. Marco de la Ilustración de Lobo */}
      <div 
        onClick={handleLoboTap}
        className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden cursor-pointer shadow-2xl border border-slate-800 bg-slate-900 group"
      >
        ...
        <div className="absolute top-3.5 left-3.5 z-20 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-950/75 backdrop-blur-md border border-white/20 text-white text-xs font-semibold shadow-lg">
        ...
        <div className="absolute top-3.5 right-3.5 z-20 flex items-center gap-1.5">
          <button
            onClick={nextImage}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-950/80 hover:bg-slate-900 backdrop-blur-md border border-amber-400/40 text-amber-300 text-xs font-bold shadow-lg transition-all active:scale-95"
          ...
          <div className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-full bg-slate-950/75 backdrop-blur-md border border-white/20 text-white text-xs font-medium shadow-lg">
        ...
        {/* Corazoncitos flotantes al tocar a Lobo */}
      ...
      {/* 2. Globo de Diálogo de Lobo (Con flechita apuntando a Lobo) */}
      <div className="relative w-full">
        {/* Puntero triangular del globo hacia la imagen de Lobo */}
        <div className="absolute -top-2 left-8 w-4 h-4 bg-slate-900 border-t border-l border-slate-700/70 rotate-45 z-10" />

        <div className="relative rounded-3xl bg-slate-900/95 border border-slate-800 p-4 shadow-xl backdrop-blur-md">
